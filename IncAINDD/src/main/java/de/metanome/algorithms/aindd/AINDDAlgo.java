...
public class AINDDAlgo extends AINDDAlgorithm implements InclusionDependencyAlgorithm,RelationalInputParameterAlgorithm,StringParameterAlgorithm, IntegerParameterAlgorithm, BooleanParameterAlgorithm {
	...
	public ArrayList<ConfigurationRequirement<?>> getConfigurationRequirements() {
        ...
        ConfigurationRequirementInteger save2disk_batch = new ConfigurationRequirementInteger(Identifier.SAVE2DISK_BATCH.name());
        Integer[] save2disk_batch2 = new Integer[]{this.save2disk_batch};
        save2disk_batch.setDefaultValues(save2disk_batch2);
        save2disk_batch.setRequired(true);
        configs.add(save2disk_batch);
        ...
        ConfigurationRequirementBoolean dataInsertion = new ConfigurationRequirementBoolean(Identifier.DATA_INSERTION.name());
        Boolean[] defaultInsertionTemp = new Boolean[]{this.dataInsertion};
        dataInsertion.setDefaultValues(defaultInsertionTemp);
        dataInsertion.setRequired(true);
        configs.add(dataInsertion);

        ConfigurationRequirementBoolean dataDeletion = new ConfigurationRequirementBoolean(Identifier.DATA_DELETION.name());
        Boolean[] defaultDeletionTemp = new Boolean[]{this.dataDeletion};
        dataDeletion.setDefaultValues(defaultDeletionTemp);
        dataDeletion.setRequired(true);
        configs.add(dataDeletion);

        ConfigurationRequirementInteger microProbingReq = new ConfigurationRequirementInteger(Identifier.MICRO_PROBING_THRESHOLD.name());
        Integer[] defaultMicroProbing = new Integer[]{this.microProbingThreshold};
        microProbingReq.setDefaultValues(defaultMicroProbing);
        microProbingReq.setRequired(true);
        configs.add(microProbingReq);
	...
	public void setRelationalInputConfigurationValue(String identifier, RelationalInputGenerator... values) throws AlgorithmConfigurationException {
		if (AINDDAlgo.Identifier.INPUT_FILES.name().equals(identifier)) {
			...
			for (int i = 0; i < tableNames.length; i++) {
    			System.out.println("File " + (i + 1) + ": " + tableNames[i]);
			}
		} else {
	...
	public void setIntegerConfigurationValue(String identifier, Integer... values) throws AlgorithmConfigurationException {
        ...
        else if (Identifier.SAVE2DISK_BATCH.name().equals(identifier)) {
            if (values[0] < 0) {
                throw new AlgorithmConfigurationException(Identifier.SAVE2DISK_BATCH.name() + " must be greater than 0!");
            }
            this.save2disk_batch = values[0];
        }
        else if (Identifier.MICRO_PROBING_THRESHOLD.name().equals(identifier)) {
            if (values[0] < 0 || values[0] > 10000) {
                throw new AlgorithmConfigurationException(Identifier.MICRO_PROBING_THRESHOLD.name() + " must be between 0 and 10000!");
            }
            this.microProbingThreshold = values[0];
        }
		else {
			this.handleUnknownConfiguration(identifier, CollectionUtils.concat(values, ","));
		}
	...
	public void setBooleanConfigurationValue(String identifier, Boolean... values) throws AlgorithmConfigurationException {
		...
		} else if (Identifier.DATA_DELETION.name().equals(identifier)) {  // 新增分支
            this.dataDeletion = values[0];
		} else if (Identifier.DATA_INSERTION.name().equals(identifier)) {  // 新增分支
            this.dataInsertion = values[0];
        } else {
			this.handleUnknownConfiguration(identifier, CollectionUtils.concat(values, ","));
		}
